...
import './admin.css';
import { AdminNavData, NavProps, ObjProps } from '../../assets/AdminNav/AdminNavData';
...
const Admin: React.FC = () => {
	...
	const handlePickMenu = useCallback((koMenu, koParentMenu, menu, parentMenu) => {
		...
		<Layout style={{ height: '100vh' }}>
			...
			<Layout>
				<Sider width={200} className="site-layout-background">
					...
					>
						<Menu.Item key={'dashboard'} title='대시보드'>
							<Link to={`/admin/dashboard/dashboard`} onClick={handleClickDashboard}>
								<span>대시보드</span>
							</Link>
						</Menu.Item>
									...
									{data.subMenu.map((elem: ObjProps) => {
										return (
											<Menu.Item key={elem.key}>
												<Link to={`/admin/${data.title}/${elem.title}`} onClick={() => handlePickMenu(elem.ko_title, data.ko_title, data.title, elem.title)}>
				...
				<Layout style={{ padding: '0 24px 24px' }}>
					...
					>
						<AdminRouteHandler param={params.param} subparam={params.subparam} />
					</Content>
				</Layout>
			</Layout>
		</Layout >
